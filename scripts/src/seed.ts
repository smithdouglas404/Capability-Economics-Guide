...
async function seedThresholds() {
  const existing = await db.select().from(capabilityThresholdsTable).limit(1);
  if (existing.length > 0) {
    console.log("Thresholds already seeded — skipping.");
    return;
  }
  const caps = await db.select({ id: capabilitiesTable.id, benchmarkScore: capabilitiesTable.benchmarkScore }).from(capabilitiesTable);
  if (caps.length === 0) {
    console.log("No capabilities found — skipping threshold seed.");
    return;
  }
  const thresholds = caps.map(c => ({
    capabilityId: c.id,
    greenMin: 70,
    yellowMin: 40,
    redMax: 39,
    description: "WEF-aligned capability maturity threshold. Green ≥ 70 (industry-leading), Yellow 40–69 (developing), Red < 40 (critical gap).",
  }));
  await db.insert(capabilityThresholdsTable).values(thresholds);
...
async function run() {
  ...
  // Technology projects, insights, leaderboard, and white papers are generated
  // on demand from Perplexity research with citations — no hardcoded seeds.
  // Trigger projects ingestion via POST /api/projects/research (admin).
}
