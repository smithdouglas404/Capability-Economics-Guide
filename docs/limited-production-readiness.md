# Limited-Production Readiness Report — Inflexcvi
...
Moving Inflexcvi off Replit onto Railway and rolling into **limited production**. Two things must be true before that's safe:
...
All six services are deployed on Railway in the **Inflexcvi** project (project ID `b4a4c027-0c13-48ad-aa90-f0c8daee52cb`, environment `production`):
...
| 4a | `scripts/src/seed-gdp-weights.ts` | Industry GDP shares (Perplexity-cited per industry, with required source URLs) | **Production** | Powers the CVI GDP-weighted rollup. Graceful-degrade if `PERPLEXITY_API_KEY` absent (CVI engine then excludes unweighted industries from the overall index). Skip with `SKIP_GDP_WEIGHTS_SEED=1`. |
| 4b | `scripts/src/seed-reference-org-rule.ts` | Single-row criterion (e.g. "top 10 per industry by revenue, mix of public + largest known private, intl coverage") in `reference_org_selection_rule` | **Production** | The rule itself is the thing you defend to a customer; the org list is derived from it. Idempotent — never overwrites; admin UI edits bump the version. Skip with `SKIP_REFERENCE_ORG_RULE_SEED=1`. |
| 4c | `scripts/src/seed-reference-orgs.ts` | Reference orgs populated by applying the criterion via Perplexity, with required `source_url` per entry | **Production** | Replaces the prior hardcoded 12-org list (Allstate/JPMC/etc.). Refresh-window guarded (default 90d); override with `FORCE_REFERENCE_ORGS_REFRESH=1`. Clean-slate via `RESET_REFERENCE_ORGS=1` (deletes rows with `sessionToken LIKE 'seed:reference:%'`; customer-added orgs untouched). Graceful-degrade if `PERPLEXITY_API_KEY` absent. Skip with `SKIP_REFERENCE_ORGS_SEED=1`. |
| 4d | `scripts/src/seed-organizations.ts` | Per-capability Perplexity scoring of each reference org (now reads org list from DB, no longer owns it) | **Production** | Scores 6-10 capabilities per org with mandatory URL citations. Drops any score that comes back without a citation. Skip with `SKIP_ORGANIZATIONS_SEED=1`. |
...
| `/security` | `pages/security.tsx` | none (intentionally public — compliance page) | Email replaced from `security@example.com` → `security@inflexcvi.ai`. |
...
| `security@example.com` replacement | **`security@inflexcvi.ai`** | Matches the valid domain already used in marketplace-seed.ts. |
