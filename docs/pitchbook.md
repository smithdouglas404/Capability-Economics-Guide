...
| Graph engine | **Neo4j (primary) with PostgreSQL fallback** for capability dependency traversals. `services/agent/graphMemory.ts` uses Cypher for `findCorrelations()` / `findRelated()` / `getGraphStats()`; falls back to Postgres on any connection error so the feature degrades gracefully. |
...
- ✅ **Neo4j graph engine** wired into `services/agent/graphMemory.ts` as the primary read path for dependency traversals, with PostgreSQL fallback on any connection error
...
**Neo4j.** Graph database (v5.26-community, self-hosted on Railway) used as the primary read path for capability dependency traversals in `services/agent/graphMemory.ts`. Functions: `findCorrelations()`, `findRelated()`, `getGraphStats()`. Activates when `NEO4J_URI`/`NEO4J_USER`/`NEO4J_PASSWORD` are set; falls back to PostgreSQL automatically on any connection error so the feature degrades gracefully.
