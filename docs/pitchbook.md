...
| Research engine | **Perplexity Sonar + Claude Sonnet 4.6 (via OpenRouter, swappable via `LLM_MODEL` env var, with Haiku → GLM-5.1 cascade on credit/budget errors)** dual-source architecture; every numeric claim traces back to either a cited URL or a deterministic computation over cited data |
...
The CE Alpha page (`pages/alpha.tsx`) is the analytical heart of the product. It exposes ten tabs, each computing a different lens on the same underlying inflexcvi dataset. Each tab is one-click reachable; each has a defined formula documented in the in-product Traceability dialog; each row carries source provenance (Perplexity citation URLs and LLM-generated rationale text) accessible via an info popover.
