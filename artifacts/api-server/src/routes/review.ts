...
async function withLockRetry<T>(label: string, fn: () => Promise<T>, maxAttempts = 30, delayMs = 5000): Promise<T | null> {
  ...
  console.error(`[review:${label}] gave up after ${maxAttempts} attempts (lock held)`);
  return null;
}

async function enrichDraftBackground(capabilityId: number, industryId: number, revisionGuidance?: string) {
  await withLockRetry("alpha", () => runAlphaEnrichment({ industryId, limitCapabilities: 1, limitEdges: 0 }));
  await withLockRetry("detail", () => runDetailEnrichment({ capabilityId, force: true, revisionGuidance }));
}
...
router.post("/review/:id/reject", async (req, res) => {
  ...
  void withLockRetry("revision", () => runDetailEnrichment({ capabilityId: id, force: true, revisionGuidance: comment }));
