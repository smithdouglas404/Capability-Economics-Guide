 ...
 * StateGraph shape as `pe-weekly-diligence.ts` but with persona-tuned
 * industry coverage, cadence, and emphasis on different action types.
 *
 * Each workflow exports a `WorkflowDefinition`; they share a generic
 * graph builder defined below to keep the file DRY. The differences
...
const CycleState = Annotation.Root({
  scannedCaps: Annotation<CycleCap[]>({ reducer: (_, y) => y, default: () => [] }),
  selectedCaps: Annotation<CycleCap[]>({ reducer: (_, y) => y, default: () => [] }),
  actionResults: Annotation<Array<{ capabilityId: number; action: string; ok: boolean; costCents: number; artifactId?: number; error?: string }>>({
    reducer: (x, y) => [...x, ...y],
    default: () => [],
  }),
  totalCostCents: Annotation<number>({ reducer: (x, y) => x + y, default: () => 0 }),
  budgetExhausted: Annotation<boolean>({ reducer: (_, y) => y, default: () => false }),
});
type Cycle = typeof CycleState.State;
...
function browseNode(cfg: PersonaCycleConfig, ctx: WorkflowRunContext) {
  return async (): Promise<Partial<Cycle>> => {
    const t0 = Date.now();
    const rows = await db
      .select({
    ...
    return { scannedCaps: rows };
  };
}

function scoreNode(cfg: PersonaCycleConfig, ctx: WorkflowRunContext) {
  return async (state: Cycle): Promise<Partial<Cycle>> => {
    const t0 = Date.now();
    const selected = state.scannedCaps.slice(0, cfg.topN);
    ...
    return { selectedCaps: selected };
  };
}

function runActionsNode(cfg: PersonaCycleConfig, ctx: WorkflowRunContext) {
  return async (state: Cycle): Promise<Partial<Cycle>> => {
    if (state.budgetExhausted || !ctx.bot) return { actionResults: [] };
    const t0 = Date.now();
...
function buildCycleGraph(cfg: PersonaCycleConfig, ctx: WorkflowRunContext) {
  return new StateGraph(CycleState)
    .addNode("browse", browseNode(cfg, ctx))
    .addNode("score", scoreNode(cfg, ctx))
    .addNode("actions", runActionsNode(cfg, ctx))
    .addEdge(START, "browse")
    .addEdge("browse", "score")
    .addConditionalEdges("score", (s) => s.budgetExhausted ? END : "actions")
    .addEdge("actions", END)
    .compile();
}
...
function buildDefinition(cfg: PersonaCycleConfig): WorkflowDefinition {
  return {
    ...
    async run(ctx: WorkflowRunContext): Promise<WorkflowResult> {
      ...
      const graph = buildCycleGraph(cfg, ctx);
      try {
        const finalState = await graph.invoke({});
        const status: WorkflowResult["status"] =
          finalState.budgetExhausted ? "budget_exhausted" :
          finalState.actionResults.some((r) => !r.ok) && finalState.actionResults.every((r) => !r.ok) ? "failed" :
          "completed";
        const artifactIds: Record<string, number[]> = {
          listings: finalState.actionResults
