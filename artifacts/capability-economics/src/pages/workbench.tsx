                        ...
                        {cards.map(card => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="font-mono text-[9px] text-muted-foreground mt-0.5 truncate">{cap?.industryName}</div>
                              <div className="flex items-center gap-1 mt-1">
                                {cap?.lifecycleStage && (
                                  <Badge variant="outline" className={`rounded-none font-mono text-[8px] uppercase tracking-wider px-1 py-0 ${LIFECYCLE_TONE[cap.lifecycleStage] ?? "bg-muted text-muted-foreground border-border/60"}`}>
                                    {cap.lifecycleStage}
                                  </Badge>
                                )}
                    ...
                    {(Object.keys(INSIGHT_KIND_LABEL) as InsightKind[]).map(kind => {
                      ...
                      return (
                        <div key={kind} className="border border-border/40 p-2 space-y-1.5">
                          ...
                          <div className="font-mono text-[9px] text-muted-foreground">{INSIGHT_KIND_HELP[kind]}</div>
                          {showPromptInput && (
                            <Input
                      ...
                      <div className="space-y-3">
                        {activeCard.insights.map(ins => (
                          <div key={ins.id} className="border border-border/40 p-2 space-y-1">
                            ...
                            <div className="font-mono text-[9px] text-muted-foreground">{ins.modelUsed} · {new Date(ins.generatedAt).toLocaleString()}</div>
                          </div>
                        ))}
                      </div>
                    </div>
