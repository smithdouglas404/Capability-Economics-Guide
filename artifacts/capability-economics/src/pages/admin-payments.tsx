...
import {
  CreditCard, FileText, Bitcoin, CheckCircle2, XCircle, Clock,
  Building2, User, Loader2, ShieldCheck, AlertTriangle, RefreshCw,
} from "lucide-react";
...
export default function AdminPaymentsPage() {
  ...
  const headers = useCallback((): HeadersInit => ({
    ...
    "x-admin-actor": user?.primaryEmailAddress?.emailAddress ?? user?.id ?? "admin",
  }), [adminKey, user]);

  const fetchAll = useCallback(async () => {
    if (!adminKey) { setError("Enter the admin key to load payments."); return; }
    setLoading(true);
    setError(null);
    try {
      const [pRes, sRes] = await Promise.all([
        fetch(`${API_BASE}/admin/payments?status=${tab}`, { headers: headers() }),
        fetch(`${API_BASE}/admin/payments/summary`, { headers: headers() }),
      ]);
  ...
  const approve = async (id: number) => {
    setActioning(id);
    try {
      const res = await fetch(`${API_BASE}/admin/payments/${id}/approve`, { method: "POST", headers: headers() });
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <header className="mb-6">
        ...
        <p className="text-muted-foreground text-sm">
          Review pending invoice and crypto requests, approve or reject access. Card payments are auto-approved on success.
        </p>
      </header>

      <Card className="mb-6 rounded-none">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-mono text-muted-foreground">Admin Key</CardTitle>
        </CardHeader>
        <CardContent>
          <Input
            type="password"
            placeholder="x-admin-key (saved locally in browser)"
            value={adminKey}
